...
class DatabaseModelMapper {
	...
	static func map(dbTopImageModel: DBTopImageModel) -> TopImageCacheModel {
		return TopImageCacheModel(etag: dbTopImageModel.etag,
								  imageData: dbTopImageModel.imageData)
	}
	...
	static func map(finOrVin: String, etag: String, imageData: Data?) -> DBTopImageModel {
		
		let dbTopImage       = DBTopImageModel()
		dbTopImage.etag      = etag
		dbTopImage.imageData = imageData
		dbTopImage.vin       = finOrVin
		return dbTopImage
	}
	...
	static func map(vehicleStatusModel: VehicleStatusModel, dbVehicleStatus: DBVehicleStatusModel) -> DBVehicleStatusModel {
		...
		dbVehicleStatus.doorLockStatusVehicle          = self.map(statusAttribute: vehicleStatusModel.vehicle.lockState, dbModel: dbVehicleStatus.doorLockStatusVehicle)
		...
		dbVehicleStatus.tankElectricLevel              = self.map(statusAttribute: vehicleStatusModel.tank.electricLevel, dbModel: dbVehicleStatus.tankElectricLevel)
	...
	private static func map(dbVehicleDealerItemList: List<DBVehicleDealerItemModel>) -> [VehicleDealerItemModel] {
		return dbVehicleDealerItemList.compactMap { (dealerItem) -> VehicleDealerItemModel? in
			...
			return VehicleDealerItemModel(dealerId: dealerItem.dealerId, role: role)
		}
	}
	...
	private static func map(dbVehicleStatusModel: DBVehicleStatusModel) -> VehicleCollisionModel {
		return VehicleCollisionModel(lastParkEvent: self.map(dbVehicleStatusIntModel: dbVehicleStatusModel.lastParkEvent),
								  ...
								  parkEventType: self.map(dbVehicleStatusIntModel: dbVehicleStatusModel.parkEventLevel))
	}
	...
	private static func map(dbVehicleStatusModel: DBVehicleStatusModel) -> VehicleTankModel {
		return VehicleTankModel(adBlueLevel: self.map(dbVehicleStatusIntModel: dbVehicleStatusModel.tankAdBlueLevel),
								electricLevel: self.map(dbVehicleStatusDoubleModel: dbVehicleStatusModel.tankElectricLevel),
	...
	private static func map(dbVehicleStatusModel: DBVehicleStatusModel) -> VehicleTheftModel {
		return VehicleTheftModel(collision: self.map(dbVehicleStatusModel: dbVehicleStatusModel),
								 ...
								 towProtectionSensorStatus: self.map(dbVehicleStatusIntModel: dbVehicleStatusModel.interiorProtectionSensorStatus))
	}
	...
	private static func map(dbVehicleStatusModel: DBVehicleStatusModel) -> VehicleVehicleModel {
		return VehicleVehicleModel(dataConnectionState: self.map(dbVehicleStatusDoubleModel: dbVehicleStatusModel.vehicleDataConnectionState),
								   ...
								   ignitionState: self.map(dbVehicleStatusIntModel: dbVehicleStatusModel.ignitionState),
								   ...
								   lockState: self.map(dbVehicleStatusIntModel: dbVehicleStatusModel.doorLockStatusVehicle),
	...
	private static func map(dbVehicleStatusModel: DBVehicleStatusModel) -> VehicleZEVModel {
		return VehicleZEVModel(chargingActive: self.map(dbVehicleStatusBoolModel: dbVehicleStatusModel.chargingActive),
	...
	private static func map<T: RawRepresentable, U: RawRepresentable>(dbVehicleStatusBoolModel model: DBVehicleStatusBoolModel?) -> StatusAttributeType<T, U> where T.RawValue == Bool, U.RawValue == Int {
		...
		guard let statusType = StatusType(rawValue: model?.status ?? -1),
			let value = model?.value.value else {
				return .invalid(timestamp: timestamp)
			}
		...
		case .invalid:	return .invalid(timestamp: timestamp)
		case .noValue:	return .noValue
		case .valid:	return .valid(value: T(rawValue: value), timestamp: timestamp, unit: self.map(dbVehicleStatusBoolModel: model))
		}
	}
	...
	private static func map(dbVehicleStatusDayTimeModel model: DBVehicleStatusDayTimeModel?) -> [DayTimeModel]? {
		...
		guard let days = model?.days, let times = model?.times else {
			return nil
		}
	...
	private static func map<T: RawRepresentable, U: RawRepresentable>(dbVehicleStatusDoubleModel model: DBVehicleStatusDoubleModel?) -> StatusAttributeType<T, U> where T.RawValue == Double, U.RawValue == Int {
		...
		case .invalid:	return .invalid(timestamp: timestamp)
		case .noValue:	return .noValue
		case .valid:	return .valid(value: T(rawValue: value), timestamp: timestamp, unit: self.map(dbVehicleStatusDoubleModel: model))
		}
	}
	...
	private static func map<T: RawRepresentable, U: RawRepresentable>(dbVehicleStatusIntModel model: DBVehicleStatusIntModel?) -> StatusAttributeType<T, U> where T.RawValue == Int, U.RawValue == Int {
		...
		case .invalid:	return .invalid(timestamp: timestamp)
		case .noValue:	return .noValue
		case .valid:	return .valid(value: T(rawValue: value), timestamp: timestamp, unit: self.map(dbVehicleStatusIntModel: model))
		}
	}
	...
	private static func map(dbVehicleStatusSocProfileModel model: DBVehicleStatusSocProfileModel?) -> [VehicleZEVSocProfileModel]? {
		...
		guard let socs = model?.socs, let times = model?.times else {
			return nil
		}
	...
	private static func map<T: RawRepresentable>(dbVehicleStatusTariffModel model: DBVehicleStatusTariffModel?) -> StatusAttributeType<[VehicleZEVTariffModel], T> where T.RawValue == Int {
	...
	private static func map(dbVehicleStatusTariffModel model: DBVehicleStatusTariffModel?) -> [VehicleZEVTariffModel]? {
		...
		guard let rates = model?.rates, let times = model?.times else {
			return nil
		}
	...
	private static func map(vehicleDealerItemModels: [VehicleDealerItemModel]) -> [DBVehicleDealerItemModel] {
