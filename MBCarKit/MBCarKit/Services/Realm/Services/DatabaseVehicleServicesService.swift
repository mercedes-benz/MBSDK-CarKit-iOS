...
public class DatabaseVehicleServicesService {
	...
	public static func resetPendingType(with finOrVin: String, onlyTrustLevel: Bool) {
		
		let pending = self.map(pendingStatus: ServicePendingState.activation)
		guard let results = self.realm.item(with: finOrVin)?.services.filter("pending = %@", pending),
			results.isEmpty == false else {
				return
		}
		
		let serviceModelRealm = RealmLayer<DBVehicleServiceModel>(config: DatabaseVehicleServicesService.config)
		serviceModelRealm.edit(results: results, editBlock: { (_, results, editCompletion) in
			...
			results.forEach { (item) in
				...
				if item.isInvalidated == false {
					if onlyTrustLevel {
						...
						if actions.contains(.updateTrustLevel) {
							item.pending = ""
						}
					} else {
						item.pending = ""
