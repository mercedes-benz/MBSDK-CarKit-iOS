...
enum BffServicesRouter: EndpointRouter {
	case get(accessToken: String, vin: String, locale: String, grouped: String?, services: [Int]?)
	...
	var httpHeaders: [String: String]? {
		...
		switch self {
		case .get(let accessToken, _, let locale, _, _),
	...
	var parameters: [String: Any]? {
		switch self {
		case .get(_, _, let locale, let grouped, let services):
			var params = [
				"locale": locale
			]
